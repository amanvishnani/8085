...
public class Parser {
    ...
    public IOpcode getOpcode(String lineInstruction) {
        ...
        String operands = parts.length > 1 ? instr.substring(parts[0].length()).trim().replace(" ", "").toUpperCase()
                : "";

        String hexOpcode = resolveOpcode(mnemonic, operands);
        if (hexOpcode == null)
            throw new RuntimeException("Instruction not found: " + instr);
    ...
    private String resolveOpcode(String mnemonic, String operands) {
        if (STATIC_OPCODES.containsKey(mnemonic))
            return STATIC_OPCODES.get(mnemonic);
        ...
        if (mnemonic.equals("INR"))
            return hex(0x04 | (REG_OFFSETS.get(operands) << 3));
        if (mnemonic.equals("DCR"))
            return hex(0x05 | (REG_OFFSETS.get(operands) << 3));
        if (ARITH_BASES.containsKey(mnemonic))
            return hex(ARITH_BASES.get(mnemonic) | REG_OFFSETS.get(operands));
    ...
    private Integer getOpcodeLength(String hex, String mnemonic) {
        if (List.of("JMP", "JZ", "JNZ", "JC", "JNC", "JP", "JM", "JPE", "JPO",
                ...
                "LDA", "STA", "LHLD", "SHLD", "LXI").contains(mnemonic))
            return 3;
        if (List.of("MVI", "ADI", "ACI", "SUI", "SBI", "ANI", "XRI", "ORI", "CPI", "IN", "OUT").contains(mnemonic))
            return 2;
        return 1;
    }

    public String extractLabel(String lineInstruction) {
        if (!lineInstruction.contains(":"))
            return "";
        return lineInstruction.split(":")[0].trim();
    }

    public IData extractData(String x) {
        Pattern p = Pattern.compile("[,\\s]\\s*([0-9A-Fa-f]{1,2})H?$", Pattern.CASE_INSENSITIVE);
    ...
    public String extractOperandLabel(IOpcode opcode) {
        ...
        if (lastPart.matches("[0-9A-Fa-f]{2,4}H?"))
            return "";
        return lastPart;
    }
